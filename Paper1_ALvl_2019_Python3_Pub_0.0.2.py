...
def Examine(Items, Characters, ItemToExamine, CurrentLocation):
    ...
    if ItemToExamine == "inventory":
        ...
        if IndexOfItem != -1:
            if Items[IndexOfItem].Name == ItemToExamine and (
                    Items[IndexOfItem].Location == INVENTORY or Items[IndexOfItem].Location == CurrentLocation):
            ...
            if Items[Count].Location == CurrentLocation:
                if len(Items[Count].Commands) >= 4:
                    if Command in Items[Count].Commands:
                        ...
                        Direction, DirectionChange = ExtractResultForCommand(Direction, DirectionChange,
                                                                             ResultForCommand)
                        ...
                        if Items[Count].ID > ID_DIFFERENCE_FOR_OBJECT_IN_TWO_LOCATIONS:
                            IndexOfOtherSideOfDoor = GetIndexOfItem("", Items[
                                Count].ID - ID_DIFFERENCE_FOR_OBJECT_IN_TWO_LOCATIONS, Items)
                        else:
                            IndexOfOtherSideOfDoor = GetIndexOfItem("", Items[
                                Count].ID + ID_DIFFERENCE_FOR_OBJECT_IN_TWO_LOCATIONS, Items)
...
def ChangeStatusOfDoor(Items, CurrentLocation, IndexOfItemToLockUnlock, IndexOfOtherSideItemToLockUnlock):
    if CurrentLocation == Items[IndexOfItemToLockUnlock].Location or CurrentLocation == Items[
        IndexOfOtherSideItemToLockUnlock].Location:
...
def UseItem(Items, ItemToUse, CurrentLocation, Places):
    ...
    if IndexOfItem != -1:
        if Items[IndexOfItem].Location == INVENTORY or (
                Items[IndexOfItem].Location == CurrentLocation and "usable" in Items[IndexOfItem].Status):
            ...
            if SubCommand == "say":
                ...
                IndexOfOtherSideItemToLockUnlock = GetIndexOfItem("", int(
                    SubCommandParameter) + ID_DIFFERENCE_FOR_OBJECT_IN_TWO_LOCATIONS, Items)
                Items = ChangeStatusOfDoor(Items, CurrentLocation, IndexOfItemToLockUnlock,
...
def GetItem(Items, ItemToGet, CurrentLocation):
    ...
    if IndexOfItem == -1:
        ...
        CanGet = False
    elif Items[IndexOfItem].Location >= MINIMUM_ID_FOR_ITEM and Items[
        GetIndexOfItem("", Items[IndexOfItem].Location, Items)].Location != CurrentLocation:
        print("You can't find " + ItemToGet + ".")
        CanGet = False
...
def CheckIfDiceGamePossible(Items, Characters, OtherCharacterName):
    ...
    while Count < len(Characters) and not PlayersInSameRoom:
        if Characters[0].CurrentLocation == Characters[Count].CurrentLocation and Characters[
            Count].Name == OtherCharacterName:
...
def PlayDiceGame(Characters, Items, OtherCharacterName):
    ...
    DiceGamePossible, IndexOfPlayerDie, IndexOfOtherCharacter, IndexOfOtherCharacterDie = CheckIfDiceGamePossible(Items,
                                                                                                                  Characters,
                                                                                                                  OtherCharacterName)
        ...
        if Items[IndexOfItem].Location == CurrentLocation:
            if len(Items[IndexOfItem].Commands) >= 4:
                if "move" in Items[IndexOfItem].Commands:
                    ...
                    ResultForCommand = GetResultForCommand(Items[IndexOfItem].Results, Position)
                    SubCommand, SubCommandParameter = ExtractResultForCommand(SubCommand, SubCommandParameter,
                                                                              ResultForCommand)
                    if SubCommand == "say":
                        Say(SubCommandParameter)
                    else:
